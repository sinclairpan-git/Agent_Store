...
- 从目录卡片的 installability 派生工作流状态：`ready_to_install`、`activation_required`、`blocked`。
...
- **FR-003-004**：workflow response 必须包含 schema_version、trace_id、error_code、audit_id、primary_action。
- **FR-003-005**：前端详情区必须展示 workflow_state、command_preview、steps、audit_id、primary/recovery action。
