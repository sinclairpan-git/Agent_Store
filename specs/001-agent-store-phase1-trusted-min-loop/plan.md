...
本计划将阶段 1 收敛为 Agent Store 侧可信最小闭环：先落 Agent Registry 草案和 Ai_AutoSDLC 官方只读页，再落手动企业激活状态、Trusted Evidence Loop、AgentOps run/session 摘要回显和用户可恢复的状态体验。计划不进入完整安装器、上架向导或质量评分。
...
| 前端技术栈 | Vue2，来自用户在 execute 阶段的技术决策 |
| 前端组件库 | SDLC 内置组件库企业 Vue2 组件库 |
...
| UX 合同 | 使用 view model / action_id / message_key / status presentation 保证可用性，不把长文案写死进证据契约 |
...
**验证方式**：schema lint、示例请求响应校验、错误码覆盖检查、Trusted Evidence Loop 一致性检查。  
**回退方式**：回退到只读官方页，不开放企业激活入口。

### Phase 1：Registry 草案与官方页

**目标**：支持 `framework.ai-autosdlc` 草案登记和官方治理视图模型渲染；前端落地时使用 Vue2 与 SDLC 内置企业 Vue2 组件库。
**产物**：Agent / AgentVersion repository、官方页 governed view model、PackageTrustSummary、EnterpriseContext。  
**验证方式**：AS-CT-001、官方页首屏字段测试、角色字段密度 snapshot。  
**回退方式**：官方页降级为静态只读草案。

### Phase 2：manual_installable-preview 与 bootstrap

**目标**：生成企业激活命令，创建 installation / device binding / signed assertion，并支持状态轮询、重试、重生命令和诊断。  
**产物**：Installation API、Device Binding API、Signed Assertion API、Bootstrap Status API、AuthContext / PermissionDecision。  
**验证方式**：AS-CT-003、AS-CT-007、AS-CT-009、AS-CT-010、AS-CT-011。  
**回退方式**：展示“可手动安装，未可信激活”，不展示实际 L5。

### Phase 3：AgentOps 摘要回显

**目标**：消费 Quality / Evidence、Approval、Runtime Policy、Credential Bootstrap summary，并把 run/session 级证据链、跨系统导航和权限失败体验纳入 contract。  
**产物**：summary client、summary cache、RunEvidenceSummary、GovernanceAssertion、官方页状态卡。  
**验证方式**：AS-CT-004、AS-CT-005、AS-CT-008、AS-CT-012、AS-CT-013。  
...
| Trusted Evidence Loop | AS-CT-008 | signed event / run / evidence / view model 一致性测试 |
| Assertion Security | AS-CT-009 | replay、revoked、audience、device key 负向测试 |
| Auth / Permission | AS-CT-010 | 客户端身份伪造拒绝测试 |
| Bootstrap Recovery | AS-CT-011 | 轮询、重试、重生命令、诊断测试 |
| Cross-System Navigation | AS-CT-012 | 权限失败、脱敏、返回路径测试 |
| State Source Guard | AS-CT-013 | 多事实源冲突降级测试 |

## 7. 实施顺序建议

1. 先实现 contract schema 和 mock contract tests。
2. 再实现 Agent / AgentVersion / Installation / DeviceBinding 数据层。
3. 再实现官方页 governed view model、PackageTrustSummary、EnterpriseContext 和状态注册表。
4. 再实现 bootstrap status/recovery、AuthContext/PermissionDecision 和 assertion security profile。
5. 再对接 AgentOps run/session summary、Trusted Evidence Loop 和 governance assertion。
6. 最后补 standalone 防过度耦合测试、可访问性测试和降级体验测试。
