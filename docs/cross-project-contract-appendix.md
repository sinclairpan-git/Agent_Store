...
| CCT-020 Store Ops deep link | AgentOps | Agent Store, Agent Store UI, Evidence Vault | Store projects `store_ops_deep_link.v1` from AgentOps run/session summary into sanitized Run Detail navigation; raw Trace/Evidence URLs are stripped and unauthorized viewers route to Evidence Vault access. |
| CCT-021 Installation distribution summary | Agent Store | Agent Store UI, AgentOps | Store emits `installation_distribution_summary.v1` as aggregate Owner distribution only; individual user/device identifiers are stripped and quality is not computed in Store. |
| CCT-022 Quality Evidence access summary | AgentOps | Agent Store, Agent Store UI, Evidence Vault | Store projects `quality_evidence_access_summary.v1` from AgentOps quality evidence into a permissioned display summary; expired `valid_until` shows “待刷新”, deprecated `score_template_id` degrades the projection, and raw Trace/Evidence URLs are stripped in favor of Evidence Vault access. |
| CCT-023 Permission denial action summary | Agent Store | Agent Store UI, AgentOps, Evidence Vault | Store emits `permission_denial_action_summary.v1` for not-visible, visible-but-not-installable, raw-evidence-denied, high-risk-approval-required, and policy-blocked pages; Store must provide actions and audit facts without issuing grants, overriding AgentOps policy, or exposing raw Trace/Evidence URLs. |
| CCT-024 Notification routing summary | Agent Store | Agent Store UI, AgentOps, Notification Center, Risk Center | Store emits `notification_routing_summary.v1` for installation, approval, feedback, lifecycle, and security events; routes remain `not_sent`, require trusted audience and audit facts, and security revocations must include the risk list. |
...
| Agent Store PRD | Reference `agentops_credential_handoff.v1`, `agent_manifest_runtime_contract.v1`, `runtime_availability_summary.v1`, `health_summary_freshness.v1`, `store_ops_deep_link.v1`, `installation_distribution_summary.v1`, `quality_evidence_access_summary.v1`, `permission_denial_action_summary.v1`, `notification_routing_summary.v1`, `installation_runtime_handoff.v1`, `draft_review_submission.v1`, `policy_approval_echo.v1`, `policy_approval_request.v1`, `policy_approval_receipt.v1`, `managed_installer_preview.v1`, `feedback_owner_response_loop.v1`, `lifecycle_governance_baseline.v1`, and `contract_registry_traceability.v1`; require external assertion field names, AgentOps credential echo, Runtime availability projection, HealthSummary freshness guard, sanitized AgentOps Run Detail navigation, aggregate installation distribution without individual identifiers, permissioned Quality/Evidence summary display, governed permission-denial action pages, audited notification routing, Runtime handoff artifact-hash binding, explicit Owner-confirmed draft review submission, AgentOps-only policy/approval authority, preview-only installer diagnostics, audited Owner feedback responses, Agent/version lifecycle governance, and contract registry traceability. |
| AgentOps PRD | Reference `signed_installation_assertion.v1`, `skill_registry_notification.v1`, `agent_manifest_runtime_contract.v1`, Store-consumed `runtime_availability_summary.v1`, Store-consumed `health_summary_freshness.v1`, Store-consumed `quality_evidence_access_summary.v1`, Store-produced `permission_denial_action_summary.v1`, Store-produced `notification_routing_summary.v1`, Runtime-consumed `installation_runtime_handoff.v1`, Store-produced `draft_review_submission.v1`, Store-consumed `policy_approval_echo.v1`, Store-produced `policy_approval_request.v1`, AgentOps-produced `policy_approval_receipt.v1`, Store-produced `managed_installer_preview.v1`, and Store-produced `contract_registry_traceability.v1`; credential issue must validate this schema and must not require assertion and device proof algorithms to be equal. |
