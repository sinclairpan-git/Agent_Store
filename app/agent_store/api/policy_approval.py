...
class PolicyApprovalEchoAPI:
    def __init__(self) -> None:
        self._idempotency: dict[str, tuple[str, dict[str, object]]] = {}

    def project_policy_approval_echo(
        self,
        payload: object,
        *,
        ...
        agentops_policy_echo = payload.get("agentops_policy_echo")
        if not isinstance(agentops_policy_echo, Mapping):
            return 400, self._validation_error(
                trace_id,
                recommended_action_id="attach_agentops_policy_echo",
                details={"reason": "agentops_policy_echo must be an object"},
            )
        ...
        response = build_policy_approval_echo(
            agent_id=agent_id,
            agent_version=agent_version,
            agentops_policy_echo=agentops_policy_echo,
            trace_id=trace_id,
            audit_id=audit_id,
        ).to_response()
