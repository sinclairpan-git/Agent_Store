...
from agent_store.domain.runtime_availability import (
    build_runtime_availability_summary,
)
...
class RuntimeAvailabilitySummaryAPI:
    def __init__(self) -> None:
        self._idempotency: dict[str, tuple[str, dict[str, object]]] = {}

    def summarize_runtime_availability(
        self,
        payload: object,
        *,
        ...
        manifest = payload.get("agent_manifest")
        if not isinstance(manifest, Mapping):
            return 400, ErrorResponse(
                ...
                severity="error",
                retryable=True,
                recommended_action_id="attach_agent_manifest",
                trace_id=trace_id,
                details={"reason": "agent_manifest must be an object"},
            ).to_dict()
        ...
        response = build_runtime_availability_summary(
            manifest,
