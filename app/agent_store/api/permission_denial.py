...
class PermissionDenialAPI:
    ...
    def summarize_denial(
        ...
        *,
        ...
        denial_context = _optional_mapping(payload, "denial_context")
        viewer_context = _optional_mapping(payload, "viewer_context")
        permission_decision = _optional_mapping(payload, "permission_decision")
        if isinstance(denial_context, ErrorResponse):
            return 400, denial_context.to_dict()
        if isinstance(viewer_context, ErrorResponse):
            return 400, viewer_context.to_dict()
        if isinstance(permission_decision, ErrorResponse):
            return 400, permission_decision.to_dict()

        audit_id = _string(payload.get("audit_id")) or f"audit-{uuid4().hex[:12]}"
        response = build_permission_denial_action_summary(
