...
from agent_store.api.installation_workflow import InstallationWorkflowAPI
...
def test_installation_workflow_api_returns_preview_for_agent() -> None:
    api = InstallationWorkflowAPI(
        sources=(
            _source(
    ...
    status, body = api.get_workflow_preview(
        "developer.release-notes", {"trace_id": "trace-workflow"}
    )

    assert status == 200
    assert response_envelope_ok(body)
    assert body["workflow"]["agent"]["agent_id"] == "developer.release-notes"
    assert body["workflow"]["workflow_state"] == "ready_to_install"


def test_installation_workflow_api_returns_stable_not_found_error() -> None:
    status, body = InstallationWorkflowAPI(sources=()).get_workflow_preview(
        "missing.agent", {"trace_id": "trace-workflow"}
    )
